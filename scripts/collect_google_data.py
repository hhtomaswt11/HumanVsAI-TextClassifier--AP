...
if not GEMINI_API_KEY:
    print("Erro: GEMINI_KEY não encontrada no ficheiro .env ou no ambiente.")
    exit(1)
...
print(f"Iniciar geração para {len(topics)} tópicos usando Gemma...")
...
with open(OUTPUT_FILE, 'a', newline='', encoding='utf-8') as csvfile:
    ...
    for idx, topic in enumerate(topics):
        ...
        for attempt in range(RETRIES_PER_TOPIC):
            texto_gerado = gerar_gemini(topic)
            ...
            if texto_gerado:
                ...
                if 70 <= wc <= 140:
                    ...
                    print(f"[Gemini] Tentativa {attempt+1} para '{topic}': texto fora do intervalo ({wc} palavras), tentando novamente...")
                    texto_gerado = None
                    time.sleep(2)
            else:
                time.sleep(2)
